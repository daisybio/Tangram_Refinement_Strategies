...
for model in ["orig","random",
    ...
    for run in range(10):
        adata_maps_pred[model][run] = an.AnnData(X=np.load(f'final_seqfish+_mouse_cortex_modifiations/{model}/{run}.npy', allow_pickle=True), 
                                                obs=adata_sc.obs,
                                                var=adata_st.obs)
...
test_genes = np.load("data/seqfish+_mouse_cortex_test_genes.npy", allow_pickle=True).tolist()
train_genes = np.load("data/seqfish+_mouse_cortex_train_genes.npy", allow_pickle=True).tolist()

adata_sc.uns["test_genes"] = adata_sc.uns["overlap_genes"]
adata_maps_true = an.read_h5ad("data/seqfish+_mouse_cortex_true_mapping_new.h5ad")

metrics = tgx.benchmark_metrics(adata_maps_pred, adata_sc, adata_st, spatial_similarity, adata_maps_true=adata_maps_true)
with open('benchmarking/seqfish+_mouse_cortex_modifiations.pkl', 'wb') as f:
