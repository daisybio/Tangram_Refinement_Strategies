...
for model in ["orig","random",
    ...
    for run in range(10):
        adata_maps_pred[model][run] = an.AnnData(X=np.load(f'../models/seqfish_mouse_embryo/{model}/{run}.npy', allow_pickle=True), 
                                                obs=adata_sc.obs,
                                                var=adata_st.obs)
        tg.cell_type_mapping(adata_maps_pred[model][run])

adata_sc.uns["test_genes"] = adata_sc.uns["overlap_genes"]
adata_maps_true = an.read_h5ad("../data/seqfish_mouse_embryo_true_mapping.h5ad")
metrics = benchmarking.eval_metrics(adata_maps_pred, adata_sc, adata_st, adata_maps_true=adata_maps_true)

with open('../metrics/seqfish_mouse_embryo.pkl', 'wb') as f:
